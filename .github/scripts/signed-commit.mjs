...
if (!created.ok) {
  if (created.status !== 422) {
    console.error(`failed to create ${branch}:`, created.payload);
    process.exit(1);
  }

  const existing = await api(`/repos/${repository}/git/ref/${ref.replace(/^refs\//, "")}`, "GET");
  if (!existing.ok || existing.payload?.ref !== ref) {
    console.error(`could not confirm existing branch ${branch}:`, existing.payload);
    process.exit(1);
  }
