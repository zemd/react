...
The maintainers of the `@zemd/react` monorepo take the security of the published packages seriously. This document sets out the terms under which security research may be conducted against this project and the process by which vulnerabilities must be reported.
...
| Package                   | Supported                |
| ------------------------- | ------------------------ |
| `@zemd/react-modals`      | Latest published release |
| `@zemd/react-slottable`   | Latest published release |
| Any prior package version | **Not supported**        |
...
1. **GitHub Private Vulnerability Reporting** (preferred) — open a report at
   <https://github.com/zemd/react/security/advisories/new>.
...
**In scope:** the source code contained in this repository and the `@zemd/react-modals` and `@zemd/react-slottable` artifacts published to npm from it.
...
The maintainers may amend this policy at any time and without notice. The version published at the default branch of <https://github.com/zemd/react> governs, and applies to any report from the moment it is submitted.
